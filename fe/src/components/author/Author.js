...
import {convertNameForApiCall} from "../context/utils";

const Author = () => {
    const name = useLocation().pathname;
    const {getDataByName, authorData, setAuthorData} = useContext(AppContext);
    const [truncatedBiography, setTruncatedBiography] = useState('');
    const [showFullBiography, setShowFullBiography] = useState(false);

    useEffect(() => {
        setAuthorData(null);
        getDataByName(convertNameForApiCall(name));
            ...
            <div className='author-header'>
                ...
                <div className='author-info'>
                    <img src={authorData.photoUrl} alt={authorData.personName}
                         title={`${authorData.personName}'s life and quotes`} loading='auto' height='100%' width='50%'/>
